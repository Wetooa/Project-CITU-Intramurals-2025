        ...
        seeds: [
          {
            ...
            team1: {
              id: "Virgo",
              name: "Virgo",
              logo: teamLogos["Virgo"],
              score: 21,
            },
            team2: {
              id: "Pisces",
              name: "Pisces",
              logo: teamLogos["Pisces"],
              score: 15,
            },
            winner: "Virgo",
            played: true,
          },
          {
            ...
            team1: {
              id: "Sagittarius",
              name: "Sagittarius",
              logo: teamLogos["Sagittarius"],
              score: 18,
            },
            team2: {
              id: "Capricorn",
              name: "Capricorn",
              logo: teamLogos["Capricorn"],
              score: 19,
            },
            winner: "Capricorn",
            played: true,
          },
          {
            ...
            team1: {
              id: "Libra",
              name: "Libra",
              logo: teamLogos["Libra"],
              score: 22,
            },
            team2: {
              id: "Scorpio",
              name: "Scorpio",
              logo: teamLogos["Scorpio"],
              score: 17,
            },
            winner: "Libra",
            played: true,
...
function MatchCard({ seed, isLast }: { seed: Seed; isLast?: boolean }) {
  return (
    <div className="w-[280px] bg-phantom_ash rounded-md overflow-hidden shadow-lg">
      ...
      >
        <div className="w-10 h-10 bg-charcoal_black rounded-full flex items-center justify-center overflow-hidden mr-3">
          <Image
        ...
        <div className="flex-1 font-medium">
          ...
          <span className="ml-1 text-xs opacity-70">
            {zodiacSignsAcronym[seed.team1.id] || ""}
          </span>
        </div>
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${
            seed.winner === seed.team1.id
              ? "bg-coral_red text-white"
              : "bg-charcoal_black"
          }`}
        >
      ...
      >
        ...
        <div className="flex-1 font-medium">
          {seed.team2.name}
          <span className="ml-1 text-xs opacity-70">
            {zodiacSignsAcronym[seed.team2.id] || ""}
          </span>
        </div>
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${
    ...
    <div className="flex flex-col items-center">
      ...
      <div className={`flex flex-col ${getSpacingClass()}`}>
        {round.seeds.map((seed, idx) => (
          <div
            key={seed.id}
            className={`relative ${idx > 0 ? "mt-8 md:mt-16" : ""}`}
          >
            <MatchCard
              seed={seed}
              isLast={isLastRound && round.seeds.length === 1}
            />
          </div>
        ))}
      </div>
    ...
    <div className="flex space-x-12 p-6 overflow-x-auto min-w-max">
      {matches.rounds.map((round, index) => (
        <div key={round.id} className="flex flex-col relative">
          <BracketRound
            round={round}
            isLastRound={index === matches.rounds.length - 1}
          />
        </div>
      ))}
    </div>
      ...
      <div className="hidden md:flex flex-col w-[300px] h-full bg-phantom_ash p-6 overflow-y-auto">
          ...
          {SPORTS.map((sport) => (
            <div
              ...
              className={`p-2 cursor-pointer transition-all hover:bg-mocha rounded ${
                selectedBracket === sport
                  ? "bg-mocha border-l-4 border-coral_red pl-3"
                  : ""
              }`}
              onClick={() => setSelectedBracket(sport)}
            >
              <span
                className={`font-medium ${selectedBracket === sport ? "text-coral_red" : ""}`}
              >
                {sport}
              </span>
            </div>
        ...
        <div className="space-y-2">
          {ESPORTS.map((esport) => (
            ...
            >
              <span
                className={`font-medium ${selectedBracket === esport ? "text-coral_red" : ""}`}
              >
                {esport}
              </span>
            </div>
            ...
            <div className="flex flex-col items-center justify-center p-8 text-center">
              <div className="bg-phantom_ash p-6 rounded-lg shadow-lg max-w-md">
                ...
                <p className="text-gray-400 mb-4">
                  There are no bracket matches available for {selectedBracket}{" "}
                  at this time. Round-robin matches may still be in progress.
                </p>
                <div className="p-2 bg-mocha rounded-md text-sm text-gray-400">
