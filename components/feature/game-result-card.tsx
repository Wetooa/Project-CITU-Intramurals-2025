...
function GameResultCard(props: Schedule) {
  const matchDateTime = props.matchDate
  ...
  return (
    <div className="rounded-2xl bg-mocha p-5 w-full flex flex-col justify-center items-center gap-2">
      ...
      <div className="bg-mocha p-5 w-full flex justify-center items-center gap-5">
        <TeamScore
          team_name="Leo"
          score={props.scoreTeam1}
          position="left"
          scoreStatus={winStatus}
        />
        <div className="flex flex-col items-center justify-center">
          <p className="font-bold lg:text-2xl">VS</p>
          <Separator />
          <p className="text-xs lg:text-xl whitespace-nowrap text-center font-medium text-gray-400">
            {matchDateTime}
          </p>
        </div>
        <TeamScore
          team_name="Capricorn"
          score={props.scoreTeam2}
          position="right"
          scoreStatus={winStatus}
        />
      </div>
...
function TeamScore({
  ...
  return (
    <div className={`flex items-center gap-5  : ""}`}>
      {position === "right" && (
        <p
          className={`text-2xl lg:text-5xl text-center ${scoreStatus === "Team_2_Win" ? "animate-pulse text-green-400" : "text-red-500"}`}
        >
          {score}
        </p>
      )}
      ...
      {position === "left" && (
        <p
          className={`text-2xl lg:text-5xl text-center ${scoreStatus === "Team_1_Win" ? "animate-pulse text-green-400" : "text-red-500"}`}
        >
          {score}
        </p>
      )}
